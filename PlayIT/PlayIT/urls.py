...
urlpatterns = patterns('',
    ...
    url(r'^pub/(?P<pk>\d+)/review/create/$', 'appPlayIT.views.review', name='review_create'),
    url(r'^pub/(?P<pkr>\d+)/review/(?P<pk>\d+)/edit/$', LoginRequiredCheckIsOwnerUpdateView.as_view(model=Review, form_class=ReviewForm), name='review_edit'),
    url(r'^pub/(?P<pkr>\d+)/review/(?P<pk>\d+)/delete/$', LoginRequiredCheckIsOwnerDeleteView.as_view(model=Review), name='review_delete'),
    ...
    url(r'^pub/(\d+)/playlist/(?P<pkr>\d+)/add_track/(?P<pk>\w+)/$', 'appPlayIT.views.add_track_to_playlist', name='add_track_to_playlist'),
    url(r'^pub/(\d+)/playlist/(?P<pkr>\d+)/remove_track/(?P<pk>\w+)/$', 'appPlayIT.views.remove_track_from_playlist', name='remove_track_from_playlist'),
    url(r'^follow_pub/(?P<pk>\d+)/$', 'appPlayIT.views.follow_pub', name='follow_pub'),
    url(r'^unfollow_pub/(?P<pk>\d+)/$', 'appPlayIT.views.unfollow_pub', name='unfollow_pub'),
